...
class utility3(commands.Cog, description="Commands that make your Discord experience nicer!"):
    ...
    #    if not self.reminding.is_running():
        self.reminding.start()
    ...
    async def reminding(self):
        for e in self.client.reminders:
            if round(e['time']) <= round(time.time()):
                ...
                embed = success_embed(
                    f"<:reminder:1049047033224384563> Reminder!",
    ...
    async def reminders(self, ctx):
        prefix = ctx.clean_prefix
        embed = discord.Embed(
            title=f"<:reminder:1049047033224384563> Your Reminders!",
            color=MAIN_COLOR
        )
    ...
    @commands.command(help="Run code and get results instantly`")
    @ignore_check()
    @commands.cooldown(3, 30, commands.BucketType.user)
    async def runhsh(self, ctx: commands.Context, *, codeblock: str = None):
        if not codeblock:
            ctx.command.reset_cooldown(ctx)
            return await ctx.reply(embed=error_embed(
                f"{EMOJIS['tick_no']} Invalid Usage!",
    ...
    async def sticker2(self, ctx: commands.Context):
...
async def setup(client):
    await client.add_cog(utility3(client))
