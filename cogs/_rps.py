...
class RPS(commands.Cog):
    ...
    async def rps(self, ctx, player2: discord.Member = None):
        ...
        if player2 == player1:
            return await ctx.reply(">>> You can not play with yourself!\nYou can play with me if you are that lonely...", ephemeral = True)
        elif player2 == None or player2 == self.bot.user:
            view = Player1Buttons()
            embed = discord.Embed(description = "Rock, paper, or scissors? Choose wisely...", color = 0x2F3136)
            await ctx.reply(embed = embed, view = view) #main message
            ...
            try:
                ...
                await ctx.send(f"> {player1.mention} ran out of time and lost.")
        elif player2.bot:
            return await send("> You can play with me or with another member. Not another bot!", ephemeral=True)
        else:
            #player 1 msg
            view = Player1Buttons()
            embed = discord.Embed(description=f"Waiting for **{player1.name}** choice..\nRock, paper, or scissors? Choose wisely...")
