...
class tictactoe(commands.Cog):
    ...
    # tictactoe command
    @commands.hybrid_command(name = "tictactoe",aliases=['ttt', 'tictac'], description = "Play TicTacToe.",usage="ttt @2ndplayer")
    @ignore_check()
    @app_commands.describe(enemy = "Player to challenge.")
    @app_commands.checks.cooldown(1, 10, key = lambda i: (i.user.id))
    async def tictactoe(self, ctx, enemy: discord.Member):
        await ctx.send(f"Tic Tac Toe: {ctx.author.mention} goes first **X**", view = TicTacToe())
