...
from prince.random import email_fun, passwords, DMs, discord_servers
from prince.constants import brain_images
from prince.ui import Paginator
#from other import topics
#from owotext import OwO
from dadjokes import Dadjoke
from discord.utils import escape_markdown
from prince.bot import Bot
from prince.custom_checks import voter_only, premium_only
...
class PressFView(discord.ui.View):
    ...
    async def press_f_nice(self, interaction: discord.Interaction, button: discord.Button):
...
class BeerView(discord.ui.View):
    ...
    @discord.ui.button(emoji='🍻')
    async def press_beer_nice(self, interaction: discord.Interaction, button: discord.Button):
...
class FreeNitroView(discord.ui.View):
    ...
    async def claim(self, interaction: discord.Interaction,_):
        await interaction.message.edit(content="https://media.discordapp.net/attachments/1106524952204681237/1106657711359275069/IMG_20220228_123449.png", embed=None, view=None)
...
class fun(commands.Cog, description="Wanna have some fun?"):
    ...
    @ignore_check()
    @commands.command(aliases=['shudi', 'should-i', 'shud_i', 'shoudi'], help="The bot will tell you what should you do!\nMake sure you separate your choices with `or`")
...
Example: `shouldi Study OR Procrastinate`
                """
            ))
...
Example: `shouldi Study OR Procrastinate`
    ...
    @commands.command(help="Pay respects! F", aliases=['press-f', 'pressf'])
  ...
  #  @commands.max_concurrency(1, per=commands.BucketType.default, wait=False)
    ...
    async def brainsize(self, ctx, user: Union[discord.Member, str] = None):
        ...
        if isinstance(user, discord.Member):
            #if user.id in [1068967045263261746]:
             #   iq = 69420
            if user.id == 866577524917534720:
                iq = 0
            else:
    ...
    async def chaty(self, ctx: commands.Context, *, message: str = None):
    ...
    @commands.command(help="Only E are allowed!", aliases=['hee'])
    @commands.cooldown(1, 10, commands.BucketType.user)
    @ignore_check()
    async def ehwhw(self, ctx: commands.Context, *, text=None):
        if not text:
            ctx.command.reset_cooldown(ctx)
...
**{escape_markdown(ctx.author.name)}** is now simping for **{escape_markdown(user.name)}**.
    ...
    async def owjjo(self, ctx, *, message=None):
        if message is None:
            ctx.command.reset_cooldown(ctx)
