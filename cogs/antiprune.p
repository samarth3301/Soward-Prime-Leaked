...
class antiprune(Cog):
    def __init__(self, client):
        self.client = client      
        self.headers = {"Authorization": f"Bot MTAxMzc3MTQ5NzE1Nzk3MjAwOA.GLxRdo.nqKSG8BrsbgiW7y8esxzGWeivkD6gddY4D7OzA"}
    ...
    async def on_member_remove(self, member) -> None:
        try:
            ...
            async for entry in guild.audit_logs(
              ...
              if str(entry.user.id) in wled or anti == "off": return 
              if user.top_role.position >= member.guild.get_member(self.client.user.id).top_role.position: return 
